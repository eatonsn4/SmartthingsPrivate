metadata {
  definition (name: "Sony Bravia TV", namespace: "NetworkGod/Sony Bravia TV", author: "Sidney Eaton") {
    capability "Switch"
    capability "Polling"
    capability "Refresh"
...
private sendXmlRpcCommand(xml) {
  ...
  def result = new physicalgraph.device.HubAction(
    method: 'POST',
    path: '/IRCC',
    body: "<?xml version=\"1.0\"?><s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\" s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"><s:Body><u:X_SendIRCC xmlns:u=\"urn:schemas-sony-com:service:IRCC:1\"><IRCCCode>${xml}</IRCCCode></u:X_SendIRCC></s:Body></s:Envelope>",
    headers: headers
  )
...
def off() {
  ...
  //def result = sendJsonRpcCommand(json)
  state.remotecommand = "AAAAAQAAAAEAAAAvAw=="
  state.button = "off"
  sendremotecommand()
...
def sendremotecommand(){
    ...
    def sonycmd = new physicalgraph.device.HubSoapAction(
            ...
            headers: [Host:"${state.tv_ip}:${tv_port}", 'X-Auth-PSK':"${tv_psk}", 'Content-Type':"application/xml"]
     )
